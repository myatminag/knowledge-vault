...
export const readCompiledTopics = (options: { vaultPath: string }) => {
  const topicsDir = path.join(options.vaultPath, "topics");
  ...
  return files.map((filePath) => {
    ...
    return {
      ...
      domain:
        typeof parsed.data.domain === "string" ? parsed.data.domain : undefined,
      topic:
